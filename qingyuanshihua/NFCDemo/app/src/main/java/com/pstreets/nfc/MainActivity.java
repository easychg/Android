...
//						Intent intenti =new Intent(MainActivity.this,IndexActivity.class);
//						//用Bundle携带数据
...
//						startActivity(intenti);
						...
						cph.setText(json.getString("by_license_plate"));
						oilname.setText(json.getString("by_oilname"));
						mz.setText(json.getString("by_gross_weight"));
						pz.setText(json.getString("by_tare_weight"));
						ghdw.setText(json.getString("by_SupplyCompany"));
						shdw.setText(json.getString("by_ReciverCompany"));
						dh.setText(json.getString("by_driver_phone"));
						lc.setText(json.getString("process_name"));
					}else{
						//弹出服务器查询信息结果
	...
	void resolveIntent(Intent intent) {
		...
		if (NfcAdapter.ACTION_TECH_DISCOVERED.equals(action)) {
			...
			MifareClassic mfc = MifareClassic.get(tagFromIntent);
				...
				icCode=new BigInteger(stringBuilder.toString(),16).toString();
				Toast.makeText(MainActivity.this,"ic卡号："+icCode,Toast.LENGTH_SHORT).show();
				progressDialog = ProgressDialog.show(MainActivity.this, "请稍等...", "获取数据中...", true);
				new Thread() {
					@Override
					public void run() {
						response = GetPostUtil.sendPost(serviceUrl+"getbyKH", "ickahao="+icCode);
						// 发送消息通知UI线程更新UI组件
